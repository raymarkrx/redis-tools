...
ifneq ($(BINARY_VERSION),)
	LDFLAGS += -X github.com/wooos/redis-tools/internal/version.version=${BINARY_VERSION}
endif
...
LDFLAGS += -X github.com/wooos/redis-tools/internal/version.gitCommit=${GIT_COMMIT}
...
.PHONY: build
build:
	GO111MODULE=on go build $(GOFLAGS) -tags '$(TAGS)' -ldflags '$(LDFLAGS)' -o $(BINDIR)/redisdump ./cmd/redisdump/
	GO111MODULE=on go build $(GOFLAGS) -tags '$(TAGS)' -ldflags '$(LDFLAGS)' -o $(BINDIR)/redisrestore ./cmd/redisrestore/
